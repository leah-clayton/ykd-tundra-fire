...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  4 13:23:11 2024
...
#%% SECTION 1: Bulk VWC burn classification from PDO product
# import shapefiles for each burn classification
shp_base = ''
...
# import shapefiles with geopandas
...
# reproject shapefiles to tif CRS
...
with rasterio.open(input_file) as src:
    # mask the tif by shapefile/burn classification
...
#%% SECTION 2: in-situ VWC
base_path = ''

fp = base_path + 'ykd_in_situ_python.xlsx'

unburned = pd.read_excel(fp, sheet_name='Unburned VWC')
burned = pd.read_excel(fp, sheet_name = '2015 Burned VWC')

unburned = unburned/100.00
burned = burned/100.00

mean_unburned = np.mean(unburned)
mean_burned = np.mean(burned)

len_unburned = len(unburned)
len_burned = len(burned)

axes[1].hist(unburned, bins=30, histtype='step', color='navy', alpha=1, linewidth=3, density=False, 
             label=f'Unburned, \n$\\bar{{x}}_u$={mean_unburned:.2f} \n(n={len_unburned})')
axes[1].axvline(mean_unburned, color='navy', linestyle='dotted', linewidth=2)

axes[1].hist(burned, bins=30, histtype='step', color='red', alpha=1, linewidth=3, density=False, 
             label=f'Burned, \n$\\bar{{x}}_b$={mean_burned:.2f} \n(n={len_burned})')
axes[1].axvline(mean_burned, color='red', linestyle='dotted', linewidth=2)
...
with rasterio.open(input_file) as src:
    ...
    # plot histograms and means
    axes[2].hist(f_raster, bins=50, color='lightgrey', alpha=0.7, density=False, 
                 label=f'Full raster, \n$\\mu$={mean_raster:.2f} \n(n={l_raster})')
    axes[2].axvline(mean_raster, color='grey', linestyle='dotted', linewidth=2)
    axes[2].hist(f_1k, bins=50, histtype='step', color='navy', alpha=1, linewidth=3, density=False, 
                 label=f'Unburned, \n$\\bar{{x}}_u$={mean_1k:.2f} \n(n={l_1k})')
    axes[2].hist(f_burn, bins=50, histtype='step', color='red', linewidth=3, alpha=1, density=False, 
                 label=f'Burned, \n$\\bar{{x}}_b$={mean_burn:.2f} \n(n={l_burn})')
    axes[2].axvline(mean_burn, color='red', linestyle='dotted', linewidth=2)
    axes[2].hist(f_re, bins=50, color='orange', histtype='step', linewidth =3, alpha=1, density=False, 
                 label=f'Reburned, \n$\\bar{{x}}_r$={mean_re:.2f} \n(n={l_re})')
    axes[2].axvline(mean_re, color='orange', linestyle='dotted', linewidth=2)
    axes[2].axvline(mean_1k, color='navy', linestyle='dotted', linewidth=2)
    

    def thousands(x, pos):
        return '{:.0f}'.format(x/1000)
    
    axes[2].yaxis.set_major_formatter(plt.FuncFormatter(thousands))
    axes[2].set_title('(c)', loc='left')
    axes[2].set_xlim(0, 1.25)
    axes[2].set_ylabel('Pixel Count (1000 pixels)')
    axes[2].set_xlabel('')
    axes[2].legend(fontsize='8', loc='upper right')
    axes[2].grid(True)

#%% SECTION 4: in-situ ALT
base_path = ''

fp = base_path + 'ykd_in_situ_python.xlsx'

unburned = pd.read_excel(fp, sheet_name='Unburned ALT match VWC')
burned = pd.read_excel(fp, sheet_name = '2015 Burned ALT')
...
axes[3].hist(unburned, bins=20, histtype='step', color='navy', alpha=1, linewidth=3, density=False, 
             label=f'Unburned, \n$\\bar{{x}}_u$={mean_unburned:.2f} \n(n={len_unburned})')
axes[3].axvline(mean_unburned, color='navy', linestyle='dotted', linewidth=2)

axes[3].hist(burned, bins=20, histtype='step', color='red', alpha=1, linewidth=3, density=False, 
             label=f'Burned, \n$\\bar{{x}}_b$={mean_burned:.2f} \n(n={len_burned})')
axes[3].axvline(mean_burned, color='red', linestyle='dotted', linewidth=2)

axes[3].set_title('(d)', loc='left')
axes[3].legend(fontsize='8', loc='upper right')
axes[3].grid(True)
axes[3].set_xlim(0, 1.25)
axes[3].set_ylabel('Observation Count')
axes[3].set_xlabel('')

#%%
save_path = base_path + 'alt_vwc_compare.png'
